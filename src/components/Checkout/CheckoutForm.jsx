...
import { FiCreditCard, FiTruck, FiChevronDown } from "react-icons/fi";
...
const stripePromise = loadStripe(
  "pk_test_51RgKLGRiWKssTbqHR3SwNnHuQl5UqySgRBVLwpoMrzth4RxlqZ5vLewbybrQVo20IAOMorMnyr9VxjrFZWc7H8xK00zmrxaSyK"
);
...
const CheckoutForm = ({ onSuccess }) => {
  ...
  function handleSelect(value) {
    handleInputChange({ target: { name: "emirate", value } }); // reuse your handler
    setIsOpen(false);
  }

  useEffect(() => {
    function onDocClick(e) {
      if (emirateRef.current && !emirateRef.current.contains(e.target)) {
        setIsOpen(false);
      }
    }
  ...
  useEffect(() => {
    if (addresses && addresses.length > 0) {
      const defaultAddress =
        addresses.find((addr) => addr.isDefault) || addresses[0]; // Use first address if no default
      if (defaultAddress) {
    ...
    <form onSubmit={handleStripePayment} className="space-y-6">
      ...
      <div>
            ...
            >
              {addresses.map((address) => (
                <option key={address._id} value={address._id}>
                  {address.isDefault ? "⭐ " : ""}
                  {address.type || "Address"} - {address.street}, {address.city}
                  , {address.emirate}
                </option>
              ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {/* ▼▼ Custom Emirate dropdown (same look, animated arrow) ▼▼ */}
          <div className="relative" ref={emirateRef}>
            <button
              ...
              className="w-full flex items-center justify-between px-4 py-3 border bg-[#f7f5f1] border-gray-300 rounded focus:outline-none focus:border-black"
            >
              <span className="text-black">
                {formData.emirate || t("checkout.emirate")}
              </span>
              <span
            ...
            <ul
              role="listbox"
              className={`absolute left-0 right-0 mt-2 border border-gray-300 rounded bg-white shadow z-10 overflow-hidden transition-all duration-200 ${
                isOpen
            ...
            >
              {emirates.map((item) => (
                <li
                  key={item}
                  role="option"
                  aria-selected={formData.emirate === item}
                  onClick={() => handleSelect(item)}
                  className="px-4 py-3 cursor-pointer text-black hover:bg-gray-100"
                >
                  {item}
                </li>
              ))}
      ...
      >
        {isProcessing
          ...
          : `${t("checkout.placeOrder")} - ${finalTotal.toFixed(2)} ${t(
              "common.currency"
            )}`}
      </button>
    </form>
